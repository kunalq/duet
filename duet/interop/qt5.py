        ...
        with impl.Scheduler() as scheduler:
            ...
            def tick():
                ...
                if scheduler.active_tasks:
                    ready_future = scheduler.ready_future
                    _call_later(self.flush_timeout, flush, ready_future)
                    _add_qt_callback(ready_future, tick)
...
def _add_qt_callback(future, func, *args, **kw):
    future.add_done_callback(lambda f: _call_later(0, func, *args, **kw))


def run(func: Callable[..., Awaitable[None]], *args, **kwds) -> None:
